...
def index():
    ...
    # get cash infom from users database
...
def buy():
    ...
    if request.method == "POST":
        ...
        # if suffucuent cash available, update users, portfolio, and histroy with the new information
        else:
...
def sell():
    ...
    if request.method == "POST"
        if not request.method.form.get("symbol") or not request.form.get("shares"):
        ...
        if quote == None:
            return apology("invalid symbol")
        if not request.form.get("shares").isdigit():
            return apology("must provide positive in")
